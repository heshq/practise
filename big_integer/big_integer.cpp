...
_create_big_integer( const unsigned *value, SYMBOL symbol, unsigned size , int is_temporary )
{
    ...
    bigint->data = ( unsigned *)malloc( size * sizeof( unsigned));
    if( bigint->data == NULL) {
        free( bigint);
        return NULL;
    }
...
big_integer_divide( BigInteger *left, BigInteger *right )
{
    ...
    //calculate
	quotient  = _create_big_integer( EMPTY_NUMBER, left->is_negative ? NEGATIVE_NUMBER: POSITIVE_NUMBER, left->size, PERSISTENT_OBJ);
    remainder = _create_big_integer( EMPTY_NUMBER, POSITIVE_NUMBER, left->size, PERSISTENT_OBJ);
...
big_integer_mod( BigInteger *left, BigInteger *right )
{
    ...
    //calculate
    quotient  = _create_big_integer( EMPTY_NUMBER, left->is_negative ? NEGATIVE_NUMBER: POSITIVE_NUMBER, left->size, PERSISTENT_OBJ);
    remainder = _create_big_integer( EMPTY_NUMBER, POSITIVE_NUMBER, right->size, PERSISTENT_OBJ);
    _big_integer_divide( left, right, quotient, remainder);
    free( quotient);
FINAL:
    if( is_temporary( left))
